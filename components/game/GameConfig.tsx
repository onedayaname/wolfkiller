...
export default function GameConfig() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-indigo-50 to-purple-100 p-4">
      <motion.div
      ...
      >
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-slate-800 mb-2">游戏配置</h1>
          <p className="text-slate-500">设置玩家数量和角色配置</p>
        </div>
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <Button
                ...
                className="h-10 w-10 rounded-full bg-indigo-500 hover:bg-indigo-600 text-white border-2 border-indigo-400 shadow-md"
              >
                -
              </Button>
              <Input
                ...
                className="w-20 text-center text-slate-800 bg-white border-2 border-indigo-200 rounded-xl [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none font-bold text-lg"
                min={6}
                max={20}
              />
              <Button
              ...
              <span className="text-slate-600 font-medium">人</span>
            </div>
          </CardContent>
        </Card>
            ...
            {ROLES.map((role) => (
              <div key={role.id} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    size="icon"
                    className="h-8 w-8 rounded-full bg-indigo-100 hover:bg-indigo-200 text-indigo-600 border border-indigo-200"
                    onClick={() => handleRoleChange(role.id, (config.roleConfig[role.id] || 0) - 1)}
                    disabled={(config.roleConfig[role.id] || 0) <= 0}
                  >
                    -
                  </Button>
                  <span className="w-8 text-center text-slate-800 font-bold text-lg">
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="p-4 bg-red-100 border border-red-300 rounded-lg text-red-700 text-center shadow-md"
          >
            {error}
          </motion.div>
        )}
